...
/**
 * <p>标题: Eb_tre_qtsf.java</p>
 * <p>业务描述:体弱儿(其它随访)(EB_TRE_QTSF)</p>
 * <p>公司:东华软件股份公司</p>
 * <p>版权:dhcc2013</p>
 * @author huangwh
 * @date 2014年4月28日
 * @version V1.0 
 */
public class Eb_tre_qtsf  implements BuildingXml {
	...
	public Map<String, String> createxml(String startTime,String endTime) {
		...
		try {
			...
			while (rs.next()) {
				...
				//随访日期
				String SFRQ = rs.getString("SFRQ");
				String VISITS_DATE = UnifieUtil.convertDateToD8( UnifieUtil.null2(SFRQ) ); //D8
				
				//随访医生
				String SFYS = rs.getString("SFYS");
				String VISITS_EMP_ID = DictMap.getCzry( UnifieUtil.null2(SFYS) ); //中心人员库 NBD03.00.001[需要注册]
				
				//随访单位
				String VISITS_ORG_CODE =  DictMap.getZcjg(rs.getString("DJJGDM"));//医疗机构代码基础字典库 NBD02.00.002[需要匹配]
				...
				String TREATMENT_GUIDELINES = UnifieUtil.null2(ZLZD).substring(0, UnifieUtil.null2(ZLZD).length()>30?30:UnifieUtil.null2(ZLZD).length());
				
				//指导意见
				String ZDYJ = rs.getString("ZDYJ");
				String GUIDANCE = UnifieUtil.null2(ZDYJ).substring(0, UnifieUtil.null2(ZDYJ).length()>30?30:UnifieUtil.null2(ZDYJ).length());
				...
				trans_no.addText("B04.01.02.12");
				
				Element request = root.addElement("resquest");
				Element trans_noChild = request.addElement("B04.01.02.12");
				...
				eVISITS_ORG_CODE.setText(VISITS_ORG_CODE);
	...
	public static void main(String[] args) {
		Eb_tre_qtsf fpsfBuildingXml = new Eb_tre_qtsf();
		fpsfBuildingXml.createxml("2009-01-01 00:00:00","2014-10-01 00:00:00");
	}
	
	private static Log logger = LogFactory.getLog(Eb_tre_qtsf.class);
